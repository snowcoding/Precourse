...
// code here
let s = document.querySelector('#createdBy');
s.innerHTML =  s.innerHTML + 'Vimal Shah';
...
function buildToDos(toDos) {
  // code here
  return toDos.map((arr,index) => {
    return buildToDo(arr,index);
  });
}
...
function displayToDos() {
  ...
  let resultArr = buildToDos(toDoItems);
  resultArr.map(el => toDoContainer.appendChild(el));
}
...
function addToDo() {
  ...
  let toDoItem = new ToDo(newToDo.value);
